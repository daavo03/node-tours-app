...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // Add new marker inside mapbox

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Adding popup to display locations

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extending the map bounds to include current location
...
var hideAlert = function hideAlert() {
  var el = document.querySelector('.alert'); // Moving 1 level up to the parent element from there remove a child element
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); // Selecting element where we want to include this html we just created

  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // Hide all alerts after 5s
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'Logged in successfully!'); // Load the home page
...
var stripe = Stripe('pk_test_51KyrOcFr1i1Q0ia97iEgBd6QuMrhLwytonOkIcuWFjQ3kqtWhkQGSvxNhgJhfcnzCwu50ULjgOtFW7GRpgKfDBpT00xlBagVZF'); // Function that take in a tour ID which comes from the user interface
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            document.querySelector('.btn--save-password').textContent = 'Save password'; // Clearing the fields after update
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  e.target.textContent = 'Processing...'; // Getting tour id from btn
